...
- 设备内容采用“左侧放大缩略图背景 + 右侧信息”的固定层级；缩略图以较低透明度向右快速渐隐，名称、配方、状态和进度全部右对齐。
- 右侧信息严格按“设备名与处理模式、当前配方、工作状态、处理百分比与剩余时间”排列。默认 32×32 画布必须完整展示四层信息；紧凑档只压缩字号与间距，不省略剩余时间。
- 正常状态不重复显示“物流通畅”等低价值文案；输入阻塞、输出阻塞、断电等异常信息优先覆盖普通状态。
- 信息必须保持单行并在空间不足时截断，不允许跨越图片区、端口栏或边框。
- 按实际可视单元格尺寸自动切换完整、紧凑、极简三档：仅在极小比例下隐藏配方和时间文字，但仍保留设备名、状态文字与进度条。
