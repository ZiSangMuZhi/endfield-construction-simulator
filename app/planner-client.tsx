...
export default function Home() {
  ...
  const machineDetail=canvasDetailScale<.5?"minimal":canvasDetailScale<.82?"compact":"full";
              ...
              {Array.from({ length: cols * rows }).map((_, index) => {
                ...
                return <button key={index} className={`cell ${cell ? `placed ${cell.kind}` : ""} ${baseCell?.kind==="belt"?"belt":""} ${pipeCell?"pipe has-pipe":""} ${status} ${cell?.root?"entity-root":""} ${cellPorts.length?"entity-port":""} ${entitySelected ? "selected-entity" : ""} ${routeSelected?"selected-route-cell":""} ${groupSelected?"group-selected-cell":""} ${entityPicked ? "picked-entity" : ""}`} aria-label={`${x},${y}${baseCell ? ` ${baseCell.kind}` : ""}${pipeCell?" pipe":""}`}
                  ...
                  onContextMenu={(e) => {e.preventDefault();if(beltBuildMode){cancelBeltDraft();return}deleteAt(x,y,hoverTransport)}}>
                    {baseCell && !isTransport(baseCell.kind) && <>{baseCell.root && <span style={footprintStyle} className={`cell-glyph root ${!machine ? "compact" : "machine-root"} ${baseCell.kind==="powerPole"?"power-pole":""} ${entitySelected?"selected-root":""} ${blockage==="input"?"input-blocked":blockage==="output"?"output-blocked":""}`}>
                      {machine?<span className="machine-content">
                        <b className="machine-visual"><AssetThumb src={machineImage} label={definition?.name??"设备"}/></b>
                        <span className="machine-overlay">
                          <span className="machine-heading">
                            <strong className="machine-name" title={machine.name}>{machine.name}</strong>
                          </span>
                          <span className="machine-recipe" title={`${modeLabel(currentRecipe!.mode)} · ${machine.recipe}`}><small className="machine-mode">{modeLabel(currentRecipe!.mode).replace("模式","")} / 配方</small><b>{machine.recipe}</b></span>
                          <span className="machine-status-line"><i className={`machine-state ${blockage!=="none"?"blocked":status}`}><span aria-hidden="true"/><b>{machine.state}</b></i></span>
                          <span className="machine-progress"><strong>{processProgress}%</strong><em>{processNote}</em><i><b className={processProgress===0?"cycle-reset":""} style={{width:`${processProgress}%`}}/></i></span>
                        </span>
